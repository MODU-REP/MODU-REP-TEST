...
export default function QCUploadPage() {
  ...
  const getConcernGuide = (name: string) => {
    if (category === "시계") {
      switch (name) {
        case "인덱스": return "12시 방향 로고가 반듯한지, 각 시간 표시 막대기(아워 마커)들이 삐뚤어지지 않고 수평이 맞는지 확인합니다.";
        case "베젤": return "시계 테두리 회전링의 눈금들이 안쪽 문자판 눈금과 일치하는지, 12시 방향의 동그란 야광점이 정중앙에 잘 맞는지 확인합니다.";
        case "날짜창": return "날짜 숫자가 위/아래/좌/우 한쪽으로 치우쳐 쏠려 있지 않은지, 글자 번짐 없이 깔끔하게 인쇄되었는지 확인합니다.";
        case "다이얼": return "문자판 안의 브랜드 로고 글씨 인쇄가 선명한지, 시침/분침 바늘이 정렬되어 있는지, 야광 도료가 골고루 예쁘게 칠해졌는지 확인합니다.";
        case "전체 검수": return "시계 앞면 유리 단차, 시계줄과 본체 사이의 연결 틈새가 너무 벌어지지 않았는지, 버클이 부드럽게 잘 잠기고 열리는지 확인합니다.";
        default: return "";
      }
    } else {
      switch (name) {
        case "로고/박임": return "브랜드 글자 로고가 삐뚤지 않고 반듯한지, 금박·은박 인쇄가 흐릿하거나 번지지 않고 선명하게 찍혔는지 확인합니다.";
        case "스티칭/대칭": return "다이아몬드 가죽 패턴(퀼팅) 라인이 대칭으로 잘 맞아떨어지는지, 바느질(박음질) 줄이 일정하고 깔끔한지 확인합니다.";
        case "가죽 질감": return "가죽 고유의 부드러운 광택과 오돌토돌한 질감(엠보싱)이 자연스러운지, 가죽 표면에 이상한 찌그러짐은 없는지 확인합니다.";
        case "하드웨어/도금": return "체인이나 금속 버클의 금빛/은빛 색상이 너무 가짜 같지 않은지, 금속 부위에 눈에 띄는 큰 스크래치(흠집)는 없는지 확인합니다.";
        case "전체 검수": return "가방을 정면에서 똑바로 보았을 때 좌우 모양 균형이 맞는지, 가방 덮개가 삐뚤어지지 않고 똑바로 잘 닫히는지 확인합니다.";
        default: return "";
      }
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (category === "시계") {
        specs = {
          dial: getSpecValue("다이얼", "문자판 안의 브랜드 로고 글씨 인쇄가 선명한지, 시침/분침 바늘이 정렬되어 있는지, 야광 도료가 골고루 예쁘게 칠해졌는지 확인합니다."),
          bezel: getSpecValue("베젤", "시계 테두리 회전링의 눈금들이 안쪽 문자판 눈금과 일치하는지, 12시 방향의 동그란 야광점이 정중앙에 잘 맞는지 확인합니다."),
          rehaut: getSpecValue("인덱스", "12시 방향 로고가 반듯한지, 각 시간 표시 막대기(아워 마커)들이 삐뚤어지지 않고 수평이 맞는지 확인합니다."),
          datewheel: getSpecValue("날짜창", "날짜 숫자가 위/아래/좌/우 한쪽으로 치우쳐 쏠려 있지 않은지, 글자 번짐 없이 깔끔하게 인쇄되었는지 확인합니다."),
          clasp: getSpecValue("전체 검수", "시계 앞면 유리 단차, 시계줄과 본체 사이의 연결 틈새가 너무 벌어지지 않았는지, 버클이 부드럽게 잘 잠기고 열리는지 확인합니다."),
        };
      } else {
        // 가방
        specs = {
          dial: getSpecValue("로고/박임", "브랜드 글자 로고가 삐뚤지 않고 반듯한지, 금박·은박 인쇄가 흐릿하거나 번지지 않고 선명하게 찍혔는지 확인합니다."),
          bezel: getSpecValue("가죽 질감", "가죽 고유의 부드러운 광택과 오돌토돌한 질감(엠보싱)이 자연스러운지, 가죽 표면에 이상한 찌그러짐은 없는지 확인합니다."),
          rehaut: getSpecValue("스티칭/대칭", "다이아몬드 가죽 패턴(퀼팅) 라인이 대칭으로 잘 맞아떨어지는지, 바느질(박음질) 줄이 일정하고 깔끔한지 확인합니다."),
          datewheel: getSpecValue("하드웨어/도금", "체인이나 금속 버클의 금빛/은빛 색상이 너무 가짜 같지 않은지, 금속 부위에 눈에 띄는 큰 스크래치(흠집)는 없는지 확인합니다."),
          clasp: getSpecValue("전체 검수", "가방을 정면에서 똑바로 보았을 때 좌우 모양 균형이 맞는지, 가방 덮개가 삐뚤어지지 않고 똑바로 잘 닫히는지 확인합니다."),
        };
      }
